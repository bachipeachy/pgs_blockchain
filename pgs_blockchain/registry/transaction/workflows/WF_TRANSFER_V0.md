...
- **Artifact Code:** WF_TRANSFER_V0
- **Artifact Kind:** workflow
- **Governed By:** CONSTITUTION_WORKFLOW_V0
- **Version:** v0
- **Status:** draft
- **Supersedes:** NONE
- **Dependencies:** IN_TRANSFER_V0, CC_RESOLVE_ACTOR_ID_V0, CC_VALIDATE_TRANSFER_POLICY_V0, CC_GENERATE_TX_ID_V0, CC_WRITE_MEMPOOL_TX_V0, CC_APPEND_TX_EVENT_V0

---

## 1. Intent

Submit a TRANSFER transaction: validate actor identity and wallet ownership, generate transaction IDs, persist to mempool, and emit lifecycle event.

---

## 2. Rationale

TRANSFER is an ENDUSER operation. The actor must be resolved and the source wallet confirmed to be owned by that actor. The BACHI model removes ETH signing — transaction IDs are content-addressable, not cryptographically signed.
...
IN_TRANSFER_V0
    ...
    │         ├─ SUCCESS → CC_VALIDATE_TRANSFER_POLICY_V0
...
| Node | Type | Purpose |
|------|------|---------|
| IN_TRANSFER_V0 | IN | Entry intent for TRANSFER |
| CC_RESOLVE_ACTOR_ID_V0 | CC | Resolve actor from actor_record |
| CC_VALIDATE_TRANSFER_POLICY_V0 | CC | Validate from_wallet ownership by actor |
...
Actor wallet must exist before transfer submission.
...
```yaml
wf_code: WF_TRANSFER_V0
version: v0
governed_by: fb.topology::CONSTITUTION_WORKFLOW_V0
runtime_binding: blockchain::RB_TRANSFER_V0
subdomain: transaction

core:
  runtime_binding: blockchain::RB_TRANSFER_V0
  summary: Submit TRANSFER transaction — validate actor ownership, persist to mempool
  ...
  nodes:
    ...
    CC_VALIDATE_TRANSFER_POLICY_V0:
      type: CC
      code: CC_VALIDATE_TRANSFER_POLICY_V0
      inputs:
